...
async function syncPositions() {
  try {
    const res = await httpsRequest(
      ...
      { headers: { 'Content-Type': 'application/json', 'User-Agent': 'GeoFleetWorker/3.4' } }
    );
    ...
    for (const v of batch) {
      for (const dateObj of [yesterday, today]) {
        ...
        try {
          ...
          const tripUrl = `https://secure.geofleet.eu/geoapi/v2.0/report/trips?apikey=${GEOFLEET_API_KEY}&id=${v.idcode}&from=${datum}&fromtime=00:00:00&to=${datum}&totime=23:59:59`;
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/status') {
    res.end(JSON.stringify({
      status: 'running', version: '3.4', uptime: process.uptime(),
    ...
    res.end(JSON.stringify({ ok: true, message: 'GeoFleet Worker v3.4 — /status /trips/sync' }));
  }
});

server.listen(PORT, () => {
  console.log(`GeoFleet Worker v3.4 listening on :${PORT}`);
  syncPositions();
